...
- Purpose: This dataset was collected as part of the 2024 Summer School project focused on Job Market Analysis.
#### Data Collection
- Source: Jobs.ch
...
- Contract Types: Festanstellung (Permanent), Praktikum (Internship), Freelancer, Lehrstelle (Apprenticeship), Temporär (Temporary) and combinations
- Workload: Represented as a percentage, indicating the minimum and maximum expected workload. The range is between 0% and 100%.
#### Data Description
...
- Missing Data: Salaries missing in some listings are planned to be predicted using a LLM.
- Duplicates: No duplicates were found in the dataset, ensuring the uniqueness of each listing.
- Wrong Values: Four instances of contract types were incorrectly scraped.
#### Data Security and Privacy
- Storage: The dataset is stored in JSON format on each team member's computer.
- Security: The data is ignored in git via gitignore. For data access contact the Team leader.
...
| 3            | contract_type | string | Festanstellung (Permanent), Praktikum (Internship), Freelancer, Lehrstelle (Apprenticeship), Temporär (Temporary) and combinations| Type of employment contract offered                                                          |
| 4            | workload      | string | 0% - 100%                                                     | Expected workload expressed as a percentage range                                                         |
| 5            | salary        | Integer | from 0 to 400'000                                             | Annual salary or hourly wage offered for the position, expressed in CHF                                                          |
...
#### Data Timeliness
- Relevance: The dataset is highly recent, scraped on July 2, 2024, making it extremely relevant for immediate analysis and use in predicting current market trends and demands.
#### Data Usability
...
#### Data Usability
- Accessibility and Extension: To access the data contact the Team leader. To extend the accessibility a scraping script could be created.
...
#### Workload Splitting:
The original 'workload' column, which ranged from 0% to 100%, has been divided into two separate columns: 'min_workload' and 'max_workload'. This split provides clearer insights into the range of expected work commitments for each position. The '%' has been removed.
#### Salary Splitting:
The original 'salary' column has been divided into two separate columns: 'min_salary' and 'max_salary'. This split provides clearer insights into the range of expected salary for each position. The CHF and '-' have been removed.
...
- DataFrame for Few-Shot Prediction: Specifically prepared for training a LLM using few-shot learning techniques to predict missing salary data. 5 Data Points were taken.
...
| 6            | Min_Salary     | Integer | From 0 to 400'000                     | Minimum annual salary in CHF             |
| 7            | Max_Salary     | Integer | From 0 to 400'000                     | Maximum annual salary in CHF             |
